...
int main()
{
    ...
    int cnt = unique(&tmp[0][1], &tmp[0][N*2+1])-tmp[0]-1;
    ...
    build(1, cnt, 1);
    //cout << "[HASH]:COUNT:" << cnt << endl ;
    for(int i = 1; i <= N; i++)
    {
        modify(1, hashmap[tmp[1][i]], hashmap[tmp[1][i+N]]-1, i);
        //cout << "Modified " << hashmap[tmp[1][i]] << " TO " << hashmap[tmp[1][i+N]] << endl;
        //cout << tmp[1][i] << " TO " << tmp[1][i+N] << endl ;
    }
    query(1, 1, cnt);
    cout << ans << endl;
    return 0;

    ...
    while(t--)
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            for(int j = 0; j < 4; j++)
            {
                if(i == j)
                    continue;
                double cosine = cvector::getCosine(arr[i], arr[j]);
                int extproduct = arr[i] ^ arr[j];
                //cerr << "Calculating :" << i << " " << j << endl ;
                //cerr << setprecision(8) << cosine << " " << extproduct << endl ;
                if(abs(cosine-0) > EPS && abs(cosine-1) > EPS && abs(cosine+1) > EPS)
                {
                    //cerr << "Requirement not satisfied 1" << endl ;
                    isRectangle = false;
                }
                ...
                {
                    //cerr << "Requirement not satisfied 2" << endl ;
                    isRectangle = false;
                }

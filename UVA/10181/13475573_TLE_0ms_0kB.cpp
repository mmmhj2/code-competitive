...
int GOAL[16][2];        // The positions of corresponding numbers in goal state
...
bool Astar(state StartState)
{
    ...
    while(!pq.empty())
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            int x = cur.x + MOVEX[i], y = cur.y + MOVEY[i];
            if(x < 0 || y < 0 || x >= 4 || y >= 4)
                continue;
            ...
            neostate.h = state::getH(neostate.board);

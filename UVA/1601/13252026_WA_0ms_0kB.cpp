...
void getInput()
{
    ...
    cin.getline(maze[0], 20);
    for(int i = 0; i < h; i++)
        cin.getline(maze[i], 20);
...
bool ForwardBFS(int stp, queue<_QueueNode> & forQue)
{
    _QueueNode curnode;
    ...
    while(!forQue.empty())          //<<
    {
        ...
        forQue.pop();
        int pos1 = curnode.gp1, pos2 = curnode.gp2, pos3 = curnode.gp3;
        if(curnode.stp > stp)           // Expand only once
        {
            //cerr << "Rejected" << endl ;
        ...
        vector<int>::iterator aitr, bitr, citr;
        //cerr << "Current pos: " << pos1 << " " << pos2 << " " << pos3 << endl ;
        for(aitr = Adj[pos1].begin(); aitr <= Adj[pos1].end(); aitr++)
        {
            int tp1 ;
            if(aitr == Adj[pos1].end())
                tp1 = pos1;
            else
                tp1 = *aitr;
            //cerr << "Trying: " << tp1 ;
            for(bitr = Adj[pos2].begin(); bitr <= Adj[pos2].end(); bitr++)
            {
                int tp2 ;
                if(bitr == Adj[pos2].end())
                    tp2 = pos2;
                else
                    tp2 = *bitr;
                ...
                if(tp1 && tp2 && tp1 == tp2)
                    continue;
                if(tp1 && tp2 && tp1 == pos2 && tp2 == pos1)
                    continue;
                //cerr << " " << tp2 ;
                for(citr = Adj[pos3].begin(); citr <= Adj[pos3].end(); citr++)
                {
                    int tp3 ;
                    if(citr == Adj[pos3].end())
                        tp3 = pos3;
                    else
                        tp3 = *citr;
                    //cerr << " " << tp3 ;
                    if((tp1 && tp3 && tp1 == tp3) || (tp2 && tp3 && tp2 == tp3))
                        continue;
                    if(tp1 && tp3 && tp1 == pos3 && tp3 == pos1)
                        continue;
                    if(tp2 && tp3 && tp2 == pos3 && tp3 == pos2)
                        continue;
...
bool ReverseBFS(int stp, queue<_QueueNode> & revQue)
{
    _QueueNode curnode;

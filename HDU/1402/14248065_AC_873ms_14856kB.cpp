...
struct cpl
{
    ...
    cpl operator * (const cpl & rhs) const
    {
        return cpl (r * rhs.r - i * rhs.i, r * rhs.i + i * rhs.r);
    }

    cpl operator + (const cpl & rhs) const
    {
        return cpl (r + rhs.r, i + rhs.i);
    }
...
class HiPreMultiply : public FastFourierTrans
{
    cplA in, in1, in2;
    cplA out, out1, out2;
    int ans[MAXN];
...
public:
    calc(const string & n1, const string & n2)
    {

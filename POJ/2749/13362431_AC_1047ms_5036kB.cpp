...
/*
inline void LinkDistSame(int a, int b, int k)
{
    // a ∧ b = F
    AddNeoEdge(a + k * n, b + (1-k) * n);
    AddNeoEdge(b + k * n, a + (1-k) * n);
}
*/

inline void LinkDistDiff(int a, int b, int k)
{
    // a ∧ !b = F
    AddNeoEdge(a + k * n, b + k * n);
    AddNeoEdge(a + (1-k) * n, b + (1-k) * n);
}
...
int check(int maxDist)
{
    ...
    for(int i = 0; i <= 2 * n; i++)
    {
        ...
        //cerr << NeoGraph[i].size() << endl ;
        //copy(NeoGraph[i].begin(), NeoGraph[i].end(), ostream_iterator<int> (cerr, " "));
        //cerr << endl ;
    }
    for(int i = 1; i <= n; i++)
        for(int j = i+1; j <= n; j++)
        {
            if(dist[i][0] + dist[j][0] > maxDist)
                AddNeoEdge(i, j+n), AddNeoEdge(j, i+n);
            if(dist[i][1] + dist[j][1] > maxDist)
                AddNeoEdge(i+n, j), AddNeoEdge(j+n, i);
            if(dist[i][0] + dist[j][1] + dist[0][0] > maxDist)
                AddNeoEdge(i, j), AddNeoEdge(j+n, i+n);
            if(dist[i][1] + dist[j][0] + dist[0][0] > maxDist)
                AddNeoEdge(i+n, j+n), AddNeoEdge(j, i);
        }
    ...
    for(int i = 1; i <= n; i++)
        ...
        {
            /*
            cerr << "Contradiction Found -- " << i << endl ;
            cerr << "Linked Edges :" << endl ;
            copy(NeoGraph[i].begin(), NeoGraph[i].end(), ostream_iterator<int> (cerr, " "));
            cerr << endl << maxDist << " " << dist[0][0] << endl ;
            */
            return false;
        }

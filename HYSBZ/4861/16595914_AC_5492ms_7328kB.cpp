...
void construct()
{
	...
	while (!Q.empty())
	{
		...
		for (int i = 0; i < 26; i++)
		{
			int & ch = E[cur].ch[i];
			if (ch)
				E[ch].fail = E[E[cur].fail].ch[i], E[ch].endCnt |=
						E[E[ch].fail].endCnt, Q.push(ch);
			else
			...
			if (dp[i][l])
				for (int j = 1; j <= N; j++)
					if (lnk[i][j] >= 0 && l + strLen[j] <= L)
						dp[lnk[i][j]][l + strLen[j]] = (1LL
								* dp[lnk[i][j]][l + strLen[j]] + dp[i][l])
								% MOD;
	i64 ans = 0;
	for (int i = 0; i <= ahoCorasick::_node::eCnt; i++)
...
const int MATRIX_SIZE = 110 * 2;
int matSiz;
int ansMat[MATRIX_SIZE][MATRIX_SIZE];
int dpMat[MATRIX_SIZE][MATRIX_SIZE];
int result[MATRIX_SIZE][MATRIX_SIZE];

void matrixMult(const int x[MATRIX_SIZE][MATRIX_SIZE],
		const int y[MATRIX_SIZE][MATRIX_SIZE])
{
	...
	for (int i = 0; i <= matSiz; i++)
		for (int j = 0; j <= matSiz; j++)
		{
			...
			for (int k = 0; k <= matSiz; k++)
				result[i][j] = (result[i][j] + (1LL * x[i][k] * y[k][j]) % MOD)
						% MOD;
		}
...
void solve()
{
	...
	{
		if (L & 1)
			matrixMult(dpMat, ansMat), memcpy(ansMat, result, sizeof result);
		matrixMult(dpMat, dpMat), memcpy(dpMat, result, sizeof result);
		L = L >> 1;
	}
...
int main()
{
	...
	for (int i = 1; i <= M; i++)
		scanf("%s", basicString[N + 1]), ahoCorasick::insert(basicString[N + 1]);

...
void DFS(int cur, int cnt)
{
	...
	vist[cur] = cnt;
	for(vector<int>::iterator itr = newAdj[cur].begin(); itr != newAdj[cur].end(); ++itr)
		if(vist[*itr] != cnt)
			DFS(*itr, cnt);
}
...
int main()
{
	...
	for(int i = 1; i <= N; i++)
	{
		...
		for(vector<int>::iterator itr = Adj[i].begin(); itr != Adj[i].end(); ++itr)
			if(SCCid[i] != SCCid[*itr] && !linked[*itr][i])
				newAdj[SCCid[i]].push_back(SCCid[*itr]), linked[*itr][i] = true;
	}
